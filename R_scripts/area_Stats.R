...
final <- read_csv("G:\\fnaExternal\\wiModEvtB.csv") # ungeneralized fire dependent vegetation areas  
...
finalFRI <- final %>% mutate(area_un = Count*30*30, acres_un = Count*30*30/4046.86, annualALL_un = (acres_un/FRI_ALLFIR), annualREPLAC = (annualALL_un*PRC_REPLAC/100), annualMIXED = (annualALL_un*PRC_MIXED/100), annualSURFAC = (annualALL_un*PRC_SURFAC/100)) 
...
write_csv(finalFRI, "G:\\fnaExternal\\wifna_FireNeeds_B.csv")

final10 <- finalFRI %>% slice (1:10) %>% 
  arrange(desc(annualALL_un))

p <- ggplot(final10) +  geom_bar(aes(x = reorder(EVT_NAME, annualALL_un), y = annualALL_un), stat = 'identity') + scale_y_continuous(limits=c(0,250000))
p + coord_flip()